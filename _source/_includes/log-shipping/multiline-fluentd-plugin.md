...
To avoid this, you can use the `fluent-plugin-concat` and customize the configuration to meet your needs. The additional configuration is added to `kubernetes.conf` (for RBAC DaemonSet) or `kubernetes-containerd.conf` (for Containerd DaemonSet).
