...
| addTimestampWithNanoSecs | Boolean. Adds `@timestamp_nano` field, to include nanoseconds in the timestamp. Set to `true` to add this field.  If you're sending multiple logs per second, we recommend setting this parameter to `true` to preserve the log sequence. | `false` |
