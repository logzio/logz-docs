...
| LOGZIO_ACCOUNT_TOKEN <span class="required-param"></span> | Required when using as a collector to ship logs to Logz.io. <br> {% include log-shipping/replace-vars.md %} <!-- logzio-inject: account-token --> |
...
| LOGZIO_LISTENER_HOST <span class="default-param">`listener.logz.io`</span> | {% include log-shipping/replace-vars-listener.md %} <!-- logzio-inject: listener-url --> |
