...
| Target namespace <span class="default-param">`kube-system`</span> | The namespace to which the Metricbeat Deamonset will be deployed. The deployment monitors the **entire** cluster, so if you already have Metricbeat installed on the `kube-system` namespace, it's recommended to change the default value. |
