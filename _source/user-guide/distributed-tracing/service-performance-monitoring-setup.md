...
The first track sends the spans back to Logz.io’s backend, and the second track extracts the metrics from the data and sends it to Logz.io’s Prometheus-based account.
...
* An active Metrics account with Logz.io (That's connected to your Tracing account)
